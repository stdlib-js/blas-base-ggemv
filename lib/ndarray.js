...
function ggemv( trans, M, N, alpha, A, strideA1, strideA2, offsetA, x, strideX, offsetX, beta, y, strideY, offsetY ) { // eslint-disable-line max-params, max-len
	...
	t = resolveStr( trans );
	if ( t === null ) {
		throw new TypeError( format( '2SpGV', trans ) );
	}
	if ( M < 0 ) {
		throw new RangeError( format( '2SpGE', M ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '2SpFz', N ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '2SpG3', strideX ) );
	}
	if ( strideY === 0 ) {
		throw new RangeError( format( '2SpHr', strideY ) );
	}
